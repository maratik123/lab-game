...
> **`origin` enforces only what a ruleset requires — read it, never assume it.** Server-side protection of `main` is decided by `gh api repos/maratik123/lab-game/rulesets`, not by this file, and not by `gh api repos/maratik123/lab-game/branches/main/protection` — that legacy endpoint answers `404: Branch not protected` even while a ruleset is active, so reading only it reports an unprotected branch that is in fact protected. Measured 2026-09-16: the repository is **public**, ruleset `main` (id 23358425) is **active**, and it carries `required_status_checks` (11 of them — Detect changes, Format, Build, Test, Race, Coverage ratchet, Test fallback, Lint, Harness guards, Actionlint, Comment references), `non_fast_forward`, `deletion`, and a `pull_request` rule with `allowed_merge_methods: ["merge"]` and 0 required approvals.
>
> **So a red required check now blocks the merge, and `origin` refuses a force-push or a deletion of `main` outright.** Re-read the ruleset rather than trusting that sentence: enforcement is a live setting, and an earlier revision of this paragraph asserted the opposite state for months after it stopped being true. What the ruleset does **not** cover stays honour-system and stays binding — force-pushing a feature branch, `git add -A`, `--no-verify`, a secret in a tracked file, and every DENY below; a green check set is not permission for any of them.
