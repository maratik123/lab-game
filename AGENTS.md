...
> **`origin` enforces only what a ruleset requires — read it, never assume it.** Server-side protection of `main` is decided by `gh api repos/maratik123/lab-game/rulesets`, not by this file, and not by `gh api repos/maratik123/lab-game/branches/main/protection` — that legacy endpoint answers `404: Branch not protected` even while a ruleset is active, so reading only it reports an unprotected branch that is in fact protected. Measured 2026-09-16: the repository is **public**, ruleset `main` (id 23358425) is **active**, and it carries `required_status_checks` (11 of them — Detect changes, Format, Build, Test, Race, Coverage ratchet, Test fallback, Lint, Harness guards, Actionlint, Comment references), `non_fast_forward`, `deletion`, a `pull_request` rule with `allowed_merge_methods: ["merge"]` and 0 required approvals, and — the field that decides what the rest of it means for you — `bypass_actors: [{actor_type: RepositoryRole, bypass_mode: pull_request}]`, with `current_user_can_bypass: "pull_requests_only"`.
>
> **So `origin` refuses a force-push or a deletion of `main` outright, but a red required check blocks the merge only for an actor who cannot bypass it — and this session's own credentials can.** `bypass_mode: pull_request` does not reach pushes, which is why the force-push half is absolute and the merge half is not; it is also why `gh pr merge … --admin` is refused by a hook below rather than left to judgement. Read `bypass_actors` whenever you read the rules: a required-check list tells you what CI must report, never who may merge past it. Re-read the ruleset rather than trusting any of this prose — enforcement is a live setting, and an earlier revision of this paragraph asserted the opposite state for months after it stopped being true. What the ruleset does **not** cover stays honour-system and stays binding — force-pushing a feature branch, `git add -A`, `--no-verify`, a secret in a tracked file, and every DENY below; a green check set is not permission for any of them.
...
> **A zero exit status is evidence about the LAST pipeline stage, not about your question.** Never pipe a gate whose exit code is load-bearing — `go test ./... | tail -6` reports `tail`'s status (always 0), so a RED gate records as green, and `tail -N` can truncate away the `FAIL` line you needed. Capture to a file **under `tmp/`** and grep the saved log: `mkdir -p tmp && go test ./... > tmp/gate.log 2>&1 && echo GATE-GREEN || echo GATE-RED`, then `grep -E "^(FAIL|ok|---)" tmp/gate.log`. `tmp/` is the one ignored scratch directory. A gate's output redirected to a bare filename — which lands in the repository ROOT — is refused by a `PreToolUse` hook, because 60 of them accumulated there unseen while this sentence named a single filename; a mutation backup or a throwaway probe written there by any other tool (`awk`, `jq`, `sed`, `cp`) is **not** matched by that hook, goes to `tmp/` all the same, and is yours to keep out. **A Go source never keeps its `.go` suffix under `tmp/`:** the directory is inside the module, so `go build ./...` compiles such a file as a package that `git status` cannot show you, and a copy whose symbols do not resolve there breaks the build for whoever runs next. Name a backup `tmp/<name>.go.bak`; put an extracted whole file (`git show <rev>:<path>`) in the session scratchpad or under a `_`-prefixed directory, which `./...` skips. A `PreToolUse` hook refuses a module-wide Go gate — `go build`/`test`/`vet` over `./...`, `golangci-lint run`, the `make` targets that compile the module — while such a file is under `tmp/`, and names it (suite `ai-docs/scripts/test-tmp-go-guard.sh`). (`set -o pipefail` also works.) A `PreToolUse` hook blocks the `go test … | tail/head` form, and a second blocks the common spelling of a `$?` read after a pipeline, not every spelling; the principle is broader than what either hook matches — the same silent-success shape covers a `jq` filter printing `null` from an error body, and a mutating flag (`rg -r`) rewriting output while exiting 0.
...
**CI runs the same gates** (`.github/workflows/ci.yml`, Go via `make`): Format · Build (build + vet + `go mod tidy` delta + the import gate) · Test · Race · Coverage ratchet · Test fallback · Lint · Harness guards (shellcheck on every script and hook body, the citation guard, the guard suites, the link check, and the check that the jobs and the checks the `main` ruleset requires are one set) · Comment references · Actionlint. Each job is `paths-filter`-gated, so **a job that did not run is not a passing job** — read the run, not the absence of red.
...
- **Delegation has FIVE phases, and failures land where attention is thinnest** — outbound (**every load-bearing claim you put INTO the prompt is executed first, whoever authored it** — a premise you originate never trips the inbound check), fit (charter *and* environment), hand-off (leave the index CLEAN, or your staged work lands in the delegate's commit), while-it-runs (a delegate waiting on a long job is waiting, not stuck), and return (**a return summary is a claim, not a record** — verify every gate/PASS against the durable artefact). Read [`ai-docs/delegation-rules.md`](ai-docs/delegation-rules.md) before any spawn that commits, edits protected files, or runs long.
...
5. **Bind the check to the COMMIT, not to the fix.** Before `git add`, ask one mechanical question of the staged set: *does this diff change a behaviour some durable document states, and is that document in this staged set?* A commit message that explains a new rule is the tell that the rule is written down somewhere else too. The act that discharges this is **reading a match list**, never recalling where a claim lives — so `grep -rn` the doc set for the **key term of the behaviour** being changed, read the matches, and re-run it after the edit: its empty output is the evidence. Then `git show --stat` the commit before considering it done, checking that every surface the message describes is in the file list. A precision fix ("make the ratio exact") propagates exactly like a correctness fix. Gates cannot see doc staleness, so only a reviewer can — and a reviewer is a slower, more expensive instrument than a grep.
...
- **Deviating from user-approved scope requires an ask, not a notification.** *"I also did X — say the word if you'd rather I revert"* puts the burden of catching scope drift on the user. Ask **before** widening scope, even when the argument is compelling — and especially when the argument comes from a reviewer whose premise you have not run. **An ask is never a route into the spec:** the owner's "yes" to extra work authorises that work in the design, recorded with their words; it becomes no Scope item, Key decision or acceptance criterion, and a question offering "do it now" and "amend the spec" as one option is malformed (`.claude/skills/task/SKILL.md`, AXIOM *the orchestrator originates no spec row*).
...
Two further cases, both earned here. A delegate that **reports its own defect** has earned trust about the *symptom*, never about the *cause* — *prefer* re-resolving its cited coordinates before authorising the fix, and extending the check to the neighbours it did not mention, because a wrong cause produces a wrong fix that now looks reviewed, and a report scoped to one line is evidence about that line only. Accepting the report and authorising the one-token fix is the frictionless move, which is the tell. And a **design's stated consequence** is a claim of exactly the same kind as a reviewer's finding: *default to* executing the path it describes against the shipped code before copying the sentence onto a second or third live surface, because the propagation obligation is what multiplies the cost of it being wrong — and when the run refutes the design, the fix goes to **every** surface already carrying the sentence, in the same change.
...
**Three shapes, each cheap to rule out.** *Prefer* running a changed guard's own fixtures against the **pre-change** artefact (`git show HEAD:<file>`) — the set of rows that flip, and only that set, is what distinguishes a load-bearing edit from a tautological test. *Prefer* reading the **cardinality of every input** before the verdict of any check shaped as *intersect two sets* / *diff against a baseline* / *grep a corpus* — an empty right-hand side makes `comm -12` and `grep -f` report the clean answer for every possible left-hand side. And where a control comes back clean across **every** variation tried, *default to* spending the next step on the channel rather than the conclusion: look for a specific in the output that could only have come from the channel you meant to close — a date, a count, or a proper noun the subject had no other way to know is the cheapest such probe.

**When the instrument is a TEST.** *Prefer* treating a green test as a claim about the test: a test earns its name as an AC's verifier only after its assertion has been pointed at the broken mechanism and seen RED. Two failure modes recur and both look like coverage — a fixture configured where the two branches coincide, and a test that hand-builds the object under test instead of going through the wiring that constructs it. *Prefer* driving the public entry point over constructing internals, and confirm the mutant **builds** as a separate step before reading the test result: a mutant that changes the shape of the program tests the compiler, not the suite, so a non-zero exit is not "the test caught it" until the failure line names the assertion. Where the apparatus and the subject are the same file — a fix whose deliverable IS the test's discriminating power — the pair is not "fails, then passes": under a probe inducing a failure class the test is **not** about, it must STAY red, and the success criterion is that the failure line now NAMES the class.

**And in its probabilistic shape.** For a defect reported as intermittent at a stated rate, one green run is one Bernoulli trial, not a reproduction attempt. *Prefer* running enough trials that P(all green) is small, and reading **which** test failed rather than only whether the suite did — a pass/fail summary hides the case where a neighbouring test is the same defect wearing a different symptom. Its red twin: when two load generators run concurrently, neither one's name describes the condition — the condition is their **sum**, and a per-instance rate computed under both belongs to neither alone.

**And in its RED direction, which the shapes above do not cover.** *Prefer* treating **any** result the apparatus could have produced regardless of the subject as evidence about the apparatus — a RED whose text describes the *instrument* qualifies exactly as an empty set does. A guard can only report the failure mode it was taught to name, so *"scan clean"* beside a red result is the shape that most invites fixing the wrong thing: the clean scan is a claim about the scan's **vocabulary**, not about the run. *Prefer* putting a sampler on the instrument while it runs and reading what it says, over reasoning from either the report's stated mechanism or the gate's own verdict. Both directions share one cheap discharge: read a cardinality the filter must find non-zero, or re-run the same command against the pre-change artefact and compare.
...
Validated in this project (`ai-docs/learnings.md`, 2026-08-30, 2026-08-31 ×2 and 2026-09-12 ×2, all `Kind: validation`): a piped-gate guard whose pre-edit re-run flipped exactly the eleven expected rows; four `/improve` eval baselines whose uniform GREEN was traced to the pre-change tree instructing every baseline agent to read the source correction — caught by a date in a returned answer that the cited command does not print; an `/ai-audit` Checklist O whose `comm -12` had always been empty because its extractor had never matched anything; a coverage-profile filter whose empty output would have read as "the branch has no blocks" until a cardinality in the same profile showed it splitting the wrong field, paired with a RED gate re-run on the pristine tree that attributed it to a pre-existing sizing limit rather than to the change; and a contention gate printing "exhaustion scan clean" beside a red result, where a 0.5 s sampler on the container's mount replaced the whole hypothesis space in two minutes.
...
**Two logs, one genre each.** `ai-docs/learnings.md` holds **conduct corrections and validations**; it is `/improve`'s input. A harness diagnosis (a gap or defect in an instruction file) goes to [`ai-docs/harness-gaps.md`](ai-docs/harness-gaps.md) — same skeleton plus a required `target:` field. That log is no flow's input here: the harness supervisor the owner runs reads it outside this repository, and a fix arrives either as a forge patch — a `harness/forge-<N>` branch the owner merges — or through a flow the owner runs for it. Two closing fields record which, and only a forge writes either, even when the fix came from a flow here (the log's header has the rule; CI gates it): `**Forge:**` names the forge that took the entry, `**Closed by:**` the pull request that fixed it in full outside any forge. An entry about **another entry** belongs in neither: use the original's `Superseded by:` field (Boundary rule 1's exception). Misfiled genre = violation.
