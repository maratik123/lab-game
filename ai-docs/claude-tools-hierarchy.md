...
| `PreToolUse` | `Edit\|Write\|MultiEdit\|NotebookEdit` | instruction-edit guard (Edit side) | Same rule keyed on `tool_input.file_path`, same two-state condition, same message. |
...
|---|---|---|
| `.claude/skills/ai-audit/scripts/check-citations.sh` | `/ai-audit` Phase 2, or standalone | Every `#N` / learnings-date / memory-file citation must resolve **for its reader**: local refs resolve here, inherited ones name `graphite-gp` or `quartzite`. Exit 1 on any unresolvable citation. |
| `.claude/skills/ai-audit/scripts/test-check-citations.sh` | before editing the guard | Locks the content-addressed (never line-pinned) exclusion, and that the guard restores the tree it edits. |
...
| `ai-docs/scripts/test-instruction-edit-guard.sh` | before editing the instruction-edit hook, and by CI's Harness-guards job | 38 fixtures through both hook bodies **extracted from `.claude/settings.json`**, run in a scratch directory set to each of the guard's three states (live / in-flight / no interview): the real 2026-09-08 `write_text` command and its `Edit` equivalent must block; reads, copies out, the `git checkout --` that undid it, and every log/plans write must pass. |
