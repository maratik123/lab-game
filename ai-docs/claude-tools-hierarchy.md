...
| `ai-docs/scripts/doc-edit-guard.sh` | `spec-writer` and `design-writer` around every scripted edit of their document | `snapshot <file>` copies to `<file>.bak` and counts `## ` headings, `| AC` rows, `| D`/`| KD` rows and code fences; `verify <file>` restores from the copy and exits 2 when any count shrank — the heading-anchored slice that truncated a design (2026-09-02) and a spec (2026-09-08). |
