...
- Integration tests run against a real Postgres provisioned by `internal/testdb`: a `docker.io/library/postgres:18` container through testcontainers-go (major tag only — the standing image policy, [`key-decisions.md`](key-decisions.md) KD-19), or the server named by `LAB_GAME_TEST_DSN`. The suite never skips: with neither, `testdb.Main` exits non-zero. Each test takes its own schema from `testdb.Schema(t)` and applies the same migrations production applies (`store.Migrate`). The container's cluster lives on a tmpfs and `initdb` is told not to sync it (`POSTGRES_INITDB_ARGS=--no-sync`), because a test database that survives a crash buys nothing and syncing it cost 6 of the 8 seconds a container needed before it accepted connections — the whole suite went from ~33 s to ~13 s. `testdb.Main` also retries the container start: testcontainers shares one Ryuk reaper between the binaries `go test ./...` runs in parallel, and a binary that meets it between "created" and "running" would otherwise fail its whole package.
