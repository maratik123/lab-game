...
| A **balance** value (stamina cap, step cost, timers, shop rates, door price curve, `budget(dist)`, combat dice) — and **chunk size**, which `docs/DESIGN.md`:69 calls an *ориентир* and ships in `config/balance.yaml` as `world.chunk.cols`/`rows` | **Configuration**, per `docs/DESIGN.md` §16.5 — never a Go literal, never a Go constant |
