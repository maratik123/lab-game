...
- **Two settings are pinned, and a guard in `internal/gateguard` fails when either drifts.** `run.relative-path-mode: gomod` makes the `^cmd/` anchor mean the module root wherever the gate is invoked from; `issues.max-issues-per-linter: 0` and `issues.max-same-issues: 0` stop the gate truncating a class after its first few findings — under the defaults "every site is handled" is not a measurable claim.
...
4. **The one named exception is a scheduler handler blocked outside the database after its deadline.** The `Handler` contract covers it: a handler that issues statements on a context of its own never sees the deadline's cancellation, and no mechanism reclaims that goroutine. The exception is written into the contract rather than assumed at the call site.
