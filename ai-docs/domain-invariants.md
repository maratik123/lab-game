...
- **`ALLOWED_CHAT_IDS` is a code-level safety net**, always on in testing: the bot physically cannot message a destination the outbound gate refuses, even if snapshot sanitisation leaked (§12.5). The gate is `ingest.Gate`, installed into `internal/tg`'s per-call `Gate` seam at client construction; the client is the only route to the generated method surface, so no production path routes around it.
