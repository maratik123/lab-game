...
- **`ALLOWED_CHAT_IDS` is a code-level safety net**, always on in testing: the bot physically cannot message a destination the outbound gate refuses, even if snapshot sanitisation leaked (§12.5). The gate is `ingest.Gate`, written to occupy `internal/tg`'s per-call `Gate` seam — `tg.Options.Gate`, consulted before every attempt and before any limiter wait, with the client the only route to the generated method surface. **Nothing installs it yet:** `cmd/bot` constructs no client, so every `NewGate`/`NewPoolGate` call site today is a test. The composition root that wires it is the obligation of whichever task first sends an outbound call (#43 and the mechanics), and the seam is what makes that a one-line wiring rather than a search for every send.
