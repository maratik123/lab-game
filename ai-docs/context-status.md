...
- **Decisions worth keeping:**
  ...
  - **`make` matches as a CLASS in the piped-gate guard**, not by target enumeration. Enumeration leaks on six flag-bearing shapes that all run gates (`make -s`, `-B`, `-C .`, `-j4`, `-f Makefile`, and bare `make`), because any flag between `make` and the target breaks the anchor. The class form's only false positive is a dry run, which executes nothing — loud over-blocking beats silent under-blocking.
