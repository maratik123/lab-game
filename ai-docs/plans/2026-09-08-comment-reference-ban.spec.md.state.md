...
prior_qa:
  ...
  - round: 4
    question: "(spec amendment at /task Step 7 close. Rounds 2–3 were run by earlier sessions and their Q&A was never recorded here; the spec carries their outcomes.) KD-9 банит «любой cross-package символ» — буквально это накрывает и `time.Duration` / `errors.Is` / `context.Context` в контрактной прозе. Как трактуем имя чужого типа в комментарии?"
    answer: "Только свой модуль — гейтим. Бан сужается до символов ЭТОГО модуля вне своего пакета; stdlib и третьи пакеты разрешены — они не протухают, а протухание есть заявленная причина бана. Гейт решает класс целиком, поэтому AC2/AC3 не сужаются; сужается KD-9."
  - round: 4
    question: "AC15 буквально запрещает обвязку гейта (`Makefile`, `ci.yml`, `pre-commit`), которую требуют пункты Scope 5 и 6 той же спеки."
    answer: "AC15 — про свип. Файлы, меняющиеся ради обвязки, симлинка или `--help`, в него не входят; carve-out делается явным в самом критерии."
  - round: 4
    question: "`find` в джобе Harness-guards достаёт только до `.claude` и `ai-docs/scripts` — `.githooks/**` в CI не шеллчекается, хотя AGENTS.md § Build & Test утверждает обратное. Этот PR добавляет туда `pre-commit.sh`."
    answer: "В скоуп — починить здесь. Стало Scope item 11, KD-18 и AC24."
```
