...
4. Connectivity guaranteed by construction at both of the design's two levels: a spanning structure over a chunk's cells derived from the chunk seed, and guaranteed portals on every border between neighbouring chunks. [task: "inside a chunk, a spanning structure from the chunk seed (a classic maze algorithm over a finite set — connectivity by construction)"] [task: "1-2 guaranteed portals per border"]
5. Passages beyond the spanning structure, at a share generation takes as an input, so a chunk holds cycles rather than a single path between any two of its cells. [task: "10-20% extra passages for cycles"]
6. Unreachable islands, at a share generation takes as an input. [task: "Unreachable islands are allowed (a single cell walled on six sides, or a closed group) and their share is a biome parameter."] [task: "the island share as a parameter"]
7. The hook a prefab layer plugs into: a prefab-membership decision for a coordinate that precedes fabric generation for it. [task: "The generator must expose the hook prefabs plug into (a prefab check precedes fabric generation, §2.2.3)."]
8. The cost of generating one cell is measured, so whether a cache is required becomes a question with evidence behind it. [task: "Benchmarks: generating a cell must be cheap enough to call on every step without caching being mandatory."]
9. Which maze algorithm runs inside a chunk stops being an open question wherever a live document calls it one. [task: "the open question this issue closes"]

## Out of scope
- Prefab content, the prefab definition format, and the world/biome config the generator's parameters come from — #28 owns them; only the hook they plug into lands here. [task: "Prefabs and the world/biome config — #28."]
...
| Question | Decision |
|---|---|
| Which maze algorithm runs inside a chunk? | TBD — round-1 question 1. |
| What does generation yield for a cell besides its six face states? | TBD — round-1 question 2. |
| What makes "cheap enough to call on every step without caching being mandatory" met? | TBD — round-1 question 3. |
...
| AC14 | No live document still states the choice of intra-chunk maze algorithm as an open question — `docs/DESIGN.md` §16.2 is one such site and does not bound the class, per AGENTS.md § *Propagation Rule* step 4. [task: "the open question this issue closes"] |
| AC15 | TBD — what "cheap enough to call on every step without caching being mandatory" requires (round-1 question 3). |

## Open questions
- None beyond the three TBD rows above, which are this round's questions.
