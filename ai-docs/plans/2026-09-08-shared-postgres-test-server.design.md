...
**Citation rule for this document.** Every factual sentence carries exactly one tag.
A fact about something that already exists carries `[measured <pin>:<path>[:<lines>] · <command> → <output>]`,
where `<pin>` is `eef4c4e` for this repository and a module version for an external
package. A claim about an artefact this task will create carries `[derived → <AC or
test>]` and no coordinate, because there is nothing yet to coordinate to. No count of
files, tests, symbols or sites appears anywhere: the numbers below are decisions —
a ceiling, a bound, a timeout — and each names its source.
...
server sized for every database-backed package at once, and take the difference. On this
tree the difference has two components, and the design has to serve both
`[measured eef4c4e · go test -count=1 ./... with and without LAB_GAME_TEST_DSN, wall clock via time(1) → 14.7 s against per-package containers, 2.4 s against one already-running server; go test -count=1 ./internal/testdb alone on the container path → 7.3 s, of which the package's own tests are 0.1 s in the shared regime]`.
One container lifecycle is most of the per-package cost, and the database-backed binaries
pay it in parallel — so provisioning one container per *gate invocation* collapses those
lifecycles into a single one, while reusing a server *across* invocations removes even
that one. Decision D4 exists for the second component; without it
the pre-commit hook and every `make test` still pay one container start.
...
untouched and the child's exit status becomes the wrapper's, so no gate's status is ever
routed through a pipe `[derived → AC15]`. The wrapper never reads a gate's output and
never summarises it. Interrupt handling: the wrapper installs a signal context for
...
**D3 — the connection ceiling, term by term.** The ceiling the wrapper passes to a
server it starts is computed, not a literal, because every term but one scales with the
host's CPU count and a constant sized for a sixteen-core machine is wrong on a
sixty-four-core one:

```
ceiling = binaries × parallel × (schemaMaxConns + 1) + slack,  clamped to [imageDefault, ceilingMax]
```
...
| `parallel` | tests running simultaneously inside one binary | `[measured eef4c4e · go help testflag → "By default, -parallel is set to the value of GOMAXPROCS"]` — read at run time as `runtime.GOMAXPROCS(0)` |
...
counts the test binaries provisioning a database through this package, and D9's manifest
test keeps it honest against the tree rather than against memory `[derived → AC5]`.
...
shared memory for — and the rule when the clamp binds is to lower `-parallel`, never to
raise the ceiling further. The ceiling is passed as a server command argument; the
...
leaves the bring-up pair to the design while fixing the rule that makes either shape safe:
remove only a server this invocation started, never one it found. The locator is a pair —
...
Measured on this tree, the shift is small but systematic, and it eats most of the band the
tolerance exists to absorb
`[measured eef4c4e · go test -count=1 -covermode=atomic -coverprofile ./... in both regimes, totals summed with awk → 91.49 % against per-package containers, 91.18 % against one shared server; the differing blocks are all in internal/testdb's container branch]`
`[measured eef4c4e:ai-docs/coverage-ratchet.txt · cat ai-docs/coverage-ratchet.txt → 91.66]`
`[measured eef4c4e:.githooks/coverage-ratchet.sh:61 · sed -n '61p' .githooks/coverage-ratchet.sh → TOLERANCE_PP=0.60]`.
Left alone the mark would keep holding by a margin narrower than the run-to-run drift the
tolerance was sized for, and ordinary commits would start blocking for no reason. So the
recorded value is lowered to a value **measured on the final tree** — the implementor
takes it with `-count=1` in the shared regime rather than copying the figure above — in
the same commit as the change, with the reason in the commit message, which is one of the
two exits `AGENTS.md` sanctions. Re-measure in the CI environment too: the runner's core
...
- **An instrument** is a patience budget — how long a test waits for a condition, or a
  ...
  and the fixed waits in the observation and ingest retry suites
  `[measured eef4c4e · grep -rn 'time.Sleep\|WithTimeout\|time.After' internal/scheduler/*_test.go internal/ingest/*_test.go internal/store/*_test.go → observe_test.go and retry_test.go carry fixed sleeps and two-second deadlines]`.
...
**D10 — the propagation class, and the two surfaces that are not in it.** The class is
`AGENTS.md` § *Propagation Rule* step 4's: every live site whose claim this diff
falsifies. Members found by sweep
`[measured eef4c4e · grep -rni 'LAB_GAME_TEST_DSN\|testcontainers\|container per' --include='*.md' --include='*.sh' --include='*.yml' --include='*.json' . with ai-docs/plans excluded → ai-docs/key-decisions.md § KD-20, ai-docs/go-test-conventions.md § "Postgres is tested against Postgres", AGENTS.md § Build & Test, .githooks/coverage-ratchet.sh's runtime advice, ai-docs/context-status.md's trap entry]`.
Two exclusions, each stated so nobody re-derives them:
...
**D12 — the contention probe, and the demonstration that it can fail.** `make test-contention`
runs **one** wrapper invocation whose child starts a load run in the background against the
provisioned server and runs the race gate over the whole module in the foreground, both
logging to files under the scratch directory, killing the load run afterwards and exiting
on the foreground gate's status. The load is the database-backed packages' own tests
repeated — cross-package load by construction, needing no second implementation of
"work that hits Postgres". The recipe's shell must capture the foreground status
explicitly rather than letting the `-e` flag the Makefile sets abort it
`[measured eef4c4e:Makefile:15-17 · sed -n '15,17p' Makefile → SHELL := /bin/bash with .SHELLFLAGS := -eu -o pipefail -c]`,
and nothing is piped `[derived → AC15]`. **The probe is an instrument until it has gone
red**: before any green result is recorded, one instrument budget D9 widened is reverted
over a `cp` backup, `make test-contention` is required to go RED, and only then is the
budget restored and a green run recorded. A green probe at that step is a STOP, not a
pass — the project has run exactly this protocol before
...
| 3 | `cmd/testpg`: the wrapper — decision order, locator read/write with the reachability probe, `--up` / `--down`, signal-aware teardown, exit-status passthrough, injectable provisioner seam | `cmd/testpg/main.go`, `cmd/testpg/run.go` | 1 |
| 4 | Wrapper tests over the injected seam: no container is started by this package's own tests | `cmd/testpg/run_test.go` | 3 |
| 5 | `Makefile`: route `test` and `test-race` through the wrapper; add `test-db-up`, `test-db-down`, `test-fallback`, `test-contention` | `Makefile` | 3 |
...
| 8 | Contention tolerance per D9: classify every wall-clock constant in the database-backed suites, make the instruments tolerant, move the remaining timing assertions onto database-clock brackets | `internal/scheduler/*_test.go`, `internal/ingest/*_test.go` | 5 |
| 9 | The AC11 demonstration: revert one widened instrument over a `cp` backup, require `make test-contention` RED, restore, record the RED before any green | `internal/scheduler/*_test.go` (restored) | 8 |
...
`1..=10`. Each group is homogeneous by change-type — **code** (`*.go`, migrations, and the
executable build plumbing this change touches: `Makefile`, `.githooks/**`,
`.github/workflows/**`, and the ratchet's own recorded datum, which the hook stages in the
same commit as the code that moves it) or **instructions/harness** (`*.md`, `.claude/**`,
`AGENTS.md`, `ai-docs/**`) — never both. Same-change-type subtasks are clustered into the
...
- **Group A** — model `sonnet`, effort `medium` (pinned) via the `code-writer` subagent,
  1M-token window — subtasks 1–10 (code change-type: `*.go`, `Makefile`, `.githooks/**`,
  `.github/workflows/**`, and `ai-docs/coverage-ratchet.txt` as the gate datum subtask 10
  must stage alongside subtask 8's code). Ten subtasks, at the cap.
...
- **Group B** — model `inherit` (the orchestrator's), effort inherited from the
  ...
  inline `model=` override, 1M-token window — subtasks 11–14 (instructions/harness
...
- **The coverage regime shifts and the ratchet's remaining band is narrower than the
  drift it was sized for**, so ordinary commits start blocking. Mitigation: D7 re-centres
  the recorded mark in the same commit with a stated reason, and the implementor measures
  on the final tree instead of trusting a figure written here — `[measured eef4c4e · go test -count=1 -covermode=atomic -coverprofile ./... in both regimes → 91.49 % against per-package containers, 91.18 % against one shared server]` and `[measured eef4c4e:ai-docs/coverage-ratchet.txt · cat → 91.66]` against `[measured eef4c4e:.githooks/coverage-ratchet.sh:61 · sed -n '61p' → TOLERANCE_PP=0.60]`.
...
the child's exit status is the wrapper's, for a non-zero child `[derived → AC15]`. Fixtures:
...
container list across `make test` and see exactly the one the wrapper created
`[derived → AC1]`. AC13's landing place and AC14's agreement are read in the diff
`[derived → AC13, AC14]`.

## Open questions

- **The reading of AC18 that this design takes.** "Addressed by a locator this change
  itself defines" is read together with the spec's lifetime decision ("reuse a server the
  project's own locator already names") as requiring a locator beyond the pre-existing
  environment variable — hence D4's container-name-plus-DSN-file pair and the bring-up /
  take-down targets. Under the narrower reading, where the wrapper's own computed DSN is
  the locator, D4 and subtasks 5's two `test-db-*` targets drop out and the win shrinks to
  one container per gate invocation. Flagged rather than assumed; the design proceeds on
  the wider reading because the narrower one leaves AC4's "found rather than started"
  clause with nothing but AC2 to exercise it.
