...
prior_qa:
  ...
  - round: 3
    question: "Необратимое решение задачи: три вида ресурсов становятся членами ledger_kind одной миграцией, а член после мержа не удаляется никакой последующей. Утверждаем токены? Маршрут: ответ фиксируется в дизайне (D8), спека не меняется."
    answer: "Утвердить тройку"
  - round: 3
    question: "Правка корпуса шире, чем называет AC10: перенести радиус чанка, доли и k из списка открытых балансовых чисел §16 п.5 туда, где ими уже владеют §2.2/§2.2.2. Корпус — это DECISIONS. Маршрут: ответ фиксируется в дизайне (D13), спека не меняется."
    answer: "Перефайлить"
  - round: 3
    question: "Идентификатор мира: id из конфига — это тот самый, который персистит #29 и несёт телеметрия, или #29 заводит свой? Ревью проверило: ни #29, ни #34, ни #35 идентификатор не назначают. Маршрут: ответ фиксируется в дизайне (D3)."
    answer: "Один, из конфига"
```
