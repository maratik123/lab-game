...
prior_qa:
  ...
  - round: 3
    question: "Правка затронула четыре строки вместо названных вами AC6/AC9 — добавились Scope 3 и строка Key decisions, которые повторяли тот же абсолют. Оставляем четыре или сужаем до двух?"
    answer: "Keep all four — Одно решение применено ко всем строкам, которые его заявляли; спека остаётся внутренне согласной. Key-decisions строка хранит оба якоря, так что видно, как решение сдвинулось, а не подменилось."
```
