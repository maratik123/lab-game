...
**current_step:** Step 8 — subtask 5 of 7 complete (Group A, subtask 5 of 6)
**last_passed_gate:** go build ./... + go test -race ./... + go vet ./... + golangci-lint run (0 issues) + golangci-lint fmt -d (clean) + go mod tidy (no delta) | (subtask 5 commit, see below)
**entry_args:** 19

## Next action

**Do this immediately:** continue Group A with subtask 6 (`internal/tg` package-level guard tests — import scan for fasthttp/go-json/metrics-registry, token-absence sweep incl. the accepted `Token()`/`FileDownloadURL` exposure, literal scan for AC21, seam tests including the `telego.NewBot`/`telego.With*` source-grep, base-URL-only-in-constructor check, end-to-end call via `config.Load`-produced `BotAPIBaseURL`).
