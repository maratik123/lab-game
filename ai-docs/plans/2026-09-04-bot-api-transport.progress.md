...
**current_step:** Step 11 — review fixes complete (Round 7)
**last_passed_gate:** make verify | 2026-09-04T21:43:31Z | c438188
**entry_args:** 19
...
| R7-1 | round 7 | major | fixed@c438188 | (a) `git diff c1d03fd..HEAD -- ai-docs/learnings.md \| grep -c '^+\*\*at:\*\*'` → **1** while `grep -c '^+### '` → **2**, so `R6-6`'s `fixed@2d93871` is false and `## Decisions log` L52 records the same item as "OBJECTED, not fixed"; (b) `git log --oneline --reverse 41b39fd..HEAD` → `0adbc76 2d93871 4f1c25a`, and `R6-2`/`R6-3`/`R6-7`'s fixes are all in `0adbc76`, before the stamped `2d93871`; (c) `git log --oneline -S 'want 30s' -- internal/tg/retry_test.go` → `921d76d` and `git log --oneline -S '## Class sweep (round 4)'` → `5d7badf`, while `R5-1`…`R5-5` read `fixed@41b39fd`. All eight cells point past the fix (the narrowing direction instruction 7a scopes on). Round 6's own table still reads `⬜ Open` on all seven rows |
| R7-2 | round 7 | nit | accepted@7 — below severity floor: `ai-docs/doc-convention.md` DOC-4 forbids a `<file>.md:<line>` **design** citation; a Go-to-Go line reference in a test comment is neither, and no Go file in the tree carries a `.md:<line>` | `grep -rn '\.md:[0-9]' --include=*.go internal/ cmd/` → no hits; `sed -n '115p' internal/tg/client_test.go` → `client.go:147-149` |
| R7-3 | round 7 | nit | accepted@7 — below severity floor: the coordinates are correct on the shipped tree (re-resolved this round from a fresh coverprofile) and `R6-5`'s own cell already records that they carry no commit pin | `go test -coverprofile` over `./internal/tg/`, then `grep -n 'retry.go:3[0-9]' <profile>` → `retry.go:33.21,35.9` and `retry.go:39.18,41.3`, matching `internal/tg/retry_test.go:715,717` |
...
| `TestChatRefFromData` | (a) calls `chatRefFromData` directly, not through `Caller.Call` | **MISSED by this sweep** — found by round 7 by enumerating every `func Test` name at `fc08810` and subtracting the table's literal and glob rows; the derivation it covers is reached in production only through the caller |
...
**What was checked.** Diff `c1d03fd..HEAD` (33 files, 22 commits), `HEAD` = `4f1c25a`. Per
instruction 7a the register scoped this round: rows `R6-1` … `R6-7` all read
`fixed@2d93871`, so each row's subject was re-examined over `2d93871..HEAD` **and** each
row's own verifying command was re-run against the shipped tree. Rows `R6-8` and `R6-9`
read `accepted@6` and were not re-raised (nothing has changed on either subject: the three
zero-hit blocks in `internal/tg` are still exactly `caller.go:71.24,76.4`,
`limit.go:137.2,137.38` and `limit.go:328.16,330.3`, read from this round's own
coverprofile). The three fix commits `0adbc76`, `2d93871`, `4f1c25a` were read line by line;
`0adbc76` is 139 lines of prose and 0 lines of Go, so every factual claim it adds was
re-derived rather than read (§ *Patterns* 1).

**Row-by-row re-verification of the seven `fixed@` rows:**

- **R6-1 — genuinely fixed, and the fix is not cosmetic.** Both `loadClass` legs were mutated
  independently (`if r, ok, err := lookupRate(lookup, chatRate); err != nil { errs = … } else if ok {`
  → `if r, ok, _ := lookupRate(lookup, chatRate); ok {`, and the same on `chatCap`), and each
  turned `go test -count=1 ./internal/config/` **RED** at `--- FAIL: TestLoadTransport_Malformed`.
  The eight new rows drive all thirteen keys through `assertKeyError(t, err, ErrInvalidValue, tc.key)`,
  so each asserts the `*KeyError` names *that* key, not merely that an error occurred.
  `internal/config` now has **zero** uncovered blocks in `transport.go` (coverage 94.4 % → 95.1 %).
- **R6-4 — fixed.** `internal/tg/caller_test.go:251-264` now reports through `t.Errorf` + `return`,
  matching `internal/tgtest/tgtest.go`'s precedent, with a comment naming the reason.
- **R6-5 — fixed.** `internal/tg/retry_test.go:715,717` now cite `retry.go:33.21,35.9` and
  `retry.go:39.18,41.3`; this round's own coverprofile reports exactly those two blocks.
- **R6-2 — fixed in substance** (both rows now say *"No test states this; it is the sweep's
  reading of them"*, the second repair round 6 offered), **but stamped at the wrong commit** — see finding 1.
- **R6-7 — fixed in substance** (`← CURRENT` is gone; `R5-1` … `R5-5` no longer read `open`),
  **but the replacement stamps are wrong and the finding's own text named the right ones** — see finding 1.
- **R6-3 — NOT fixed: three of the four named omissions were added, the fourth was not.** Re-opened below.
- **R6-6 — NOT fixed, and not recorded as objected either.** Re-raised as finding 1(a).

**The class sweep's corrected claims were re-derived, not read.** Every companion the table
names resolves to a real `func Test…` (`TestLimiter_EvictRunsInsideAcquireBoundingMemory`,
`TestLimiter_LaterArrivalDoesNotJumpAnEarlierGrant`,
`TestRetry_JitterOptionThreadedThroughToBackoff`, `TestNew_InstallsPackageJSONConstructor`,
`TestNew_InstallsDiscardLoggerByDefault`); `grep -rni 'wiring' internal/tg/*_test.go
internal/tgtest/*_test.go` still returns nothing, which is now consistent with the table
rather than contradicted by it; and the new
`TestBackoffDelay_JitterBoundsExactly` row's claim was checked at the swept tree —
`git show fc08810:internal/tg/retry_test.go` shows that function asserting only
`attempt=0, jitter=0` and `attempt=0, jitter=1`, so "fixture at `attempt=0` only, where the
cap cannot bind" is true.

| # | File:line | Severity | Finding | Status |
|---|-----------|----------|---------|--------|
| 1 | (this file) `## Review register` L152-156, L159-160, L163-164 · `## Self-Review (Round 6)` table | major | **The register misreports the round-6 fix round in three ways, one of which records an OBJECTED finding as fixed — and the register is, by `ai-docs/templates/progress-format.md`, "the **only** cross-round memory the loop has".** **(a)** `R6-6` reads `fixed@2d93871`, but the fix is absent from the tree: `git diff c1d03fd..HEAD -- ai-docs/learnings.md \| grep -c '^+\*\*at:\*\*'` → **1** against `grep -c '^+### '` → **2**, i.e. the second entry this branch appends still carries no `**at:**`. This file's own `## Decisions log` L52 records the disposition as *"**Step 11 (round 6) — OBJECTED, not fixed**"* with a full rationale. So one surface says fixed, another says objected, and the round-6 table says neither (all seven of its rows still read `⬜ Open`). The status vocabulary has a cell for this — `accepted@<round> — <reason>` in the register, `⚠️ Objected: <reason>` in the round table — and using `fixed@` instead silently exempts the objection from `.claude/agents/self-review.md` § 7's objection-quality review, which is the *only* check on a fix round's decision to decline a finding. That is a guard missing its primary case, not a stale field (§ *Patterns* 3). **(b)** `R6-2`, `R6-3` and `R6-7` read `fixed@2d93871`, but `git log --oneline --reverse 41b39fd..HEAD` → `0adbc76`, `2d93871`, `4f1c25a`, and all three of those fixes (the class-sweep wording, the added sweep rows, the `← CURRENT` removal and the `R5-*` stamps) landed in `0adbc76`, the commit **before** `2d93871`. **(c)** `R5-1` … `R5-5` were stamped `fixed@41b39fd` by that same `0adbc76`, while `git log --oneline -S 'want 30s' -- internal/tg/retry_test.go` → `921d76d` and `git log --oneline -S '## Class sweep (round 4)'` → `5d7badf` — and round 6's own finding 7 stated the correct shas verbatim (*"the correct stamps are `921d76d` for `R5-1`/`R5-2`/`R5-3` and `5d7badf` for `R5-4`/`R5-5`"*), so this is not a lookup miss. Instruction 7a scopes the next round to *"only the diff since `<sha>`"*, so every one of these eight cells points **past** the commit carrying the fix — the narrowing direction, which is the unsafe one, and the opposite of `R3-3`'s wide-window error. This is the **fifth** consecutive recurrence of the stamp/status class (`R2-8`, round 4's finding 4, `R5-5`(i), `R6-7`(i)). Fix: `R6-6` → either add `**at:** 5d7badf` to that entry (making `fixed@<sha>` true) **or** record it as `⚠️ Objected: <reason>` in round 6's table and `accepted@6 — <reason>` in the register — not both surfaces disagreeing; `R6-2`/`R6-3`/`R6-7` → `fixed@0adbc76`; `R5-1`/`R5-2`/`R5-3` → `fixed@921d76d`; `R5-4`/`R5-5` → `fixed@5d7badf`; and set round 6's table Status cells the way rounds 1–5 already are. | ✅ Fixed |
| 2 | (this file) `## Class sweep (round 4)` | minor | **`R6-3` re-opened — three of its four named omissions were added, `TestChatRefFromData` was not.** Round 6's finding 3 named exactly four tests missing from the sweep table: `TestBackoffDelay_JitterBoundsExactly`, `TestChatRefFromData`, `TestNew_InstallsPackageJSONConstructor`, `TestNew_InstallsDiscardLoggerByDefault`. `0adbc76` added rows for the first, third and fourth. Measured on the shipped file: enumerating the 87 `func Test…` names at `fc08810` and subtracting every name the table states literally plus every one of its twelve glob rows (`TestSchedule_`, `TestRedFirst_`, `TestPaceWindows_`, `TestAcquireFixedPoint_`, `TestLimiter_`, `TestCaller_`, `TestRetry_`, `TestGuard_`, `TestServer_`, `TestLoadTransport_`, `TestOptionError_`, `TestError_`) leaves **exactly one** name: `TestChatRefFromData` — a real test (`grep -rl 'func TestChatRefFromData(' internal/` → `internal/tg/caller_test.go`), and no glob reaches it (`TestChatTarget_String` is a literal row, not a prefix). The finding's own words were *"A sweep whose stated purpose is to enumerate a class must say which population it enumerated"*, so the residue is the whole point. Fix: add its row (it is a pure-function table over `chatRefFromData`, form (a)/(b) both inapplicable — the same verdict as `TestClassifyMethod`). | ✅ Fixed (re-opened at round 7, now closed) |

**No Design/Spec Amendment trigger.** Both findings are edits to this progress file. No
criterion has changed and no design decision is contradicted; the spec and the design say the
right thing and every `.go` file in the tree matches them.

**Gate results re-run against the shipped artefact (not quoted from drafting):**

- `make verify` → GATE-GREEN, exit 0 (fmt, build, vet, `golangci-lint run` `0 issues.`, file-limits, `go test`, `go test -race`, tidy-check, actionlint, shellcheck). Zero `FAIL` lines in the saved log.
- `go test -count=1 -race ./internal/tg/ ./internal/tgtest/ ./internal/config/` → RACE-GREEN, uncached (`ok` ×3, 1.520 s / 1.016 s / 1.031 s).
- Coverage, re-measured this round: `internal/tg` **98.6 %**, `internal/tgtest` **80.3 %**, `internal/config` **95.1 %** (up from 94.4 % — `transport.go` now has no zero-hit block at all). The three surviving zero-hit blocks in `internal/tg` are exactly the `accepted@5` / `accepted@6` set.
- AC1: `revive`'s `exported` and `package-comments` rules are enabled in `.golangci.yml:45-48` and `golangci-lint run` reports `0 issues.`, so the doc-comment half is machine-enforced on the shipped tree. PASS.
- AC2: `grep -rnE 'fasthttp|go-json|goccy|grbit|fastjson' --include=*.go cmd/ internal/` minus tests → one hit, `internal/tg/client.go:139`, a comment. PASS.
- AC3: `go.mod:9 github.com/mymmrac/telego v1.11.2`, direct; tidy-check clean inside `make verify`. PASS.
- AC15 source half: `BaseURL` appears in exactly one non-test file, `internal/tg/client.go` (`:18,:22,:100,:101,:143`). PASS.
- AC17: no `prometheus` / `expvar` / `otel` identifier in any `internal/tg` non-test file (the only hits are in `guards_test.go`, the guard's own forbidden list). PASS.
- AC21/AC22/AC23: the malformed clause is now driven for all thirteen `LAB_GAME_TG_*` keys, and both previously-uncovered `loadClass` legs are mutation-killed (measured above). PASS.
- AC25/AC29: the design's own propagation sweep (`rg -U -n -i --glob '*.md' --glob '*.go' --glob '!ai-docs/plans/done/**' --glob '!ai-docs/plans/2026-09-04-bot-api-transport*' --glob '!ai-docs/learnings.md' --glob '!ai-docs/harness-gaps.md' 'no telegram client|none has a compiled-in default|every variable is required|compiled-in\s+(//\s*)?fallback' .`) re-run against the shipped tree: six hits, all correct — `internal/config/env.go:13` is narrowed with the transport carve-out, `config.go:41` names `Transport` as the one exception, `doc.go:13` and `balance.go:12` are balance-scoped, and `key-decisions.md` KD-22/KD-27 are the decisions themselves. No surviving falsified claim. PASS.
- AC26: no `api_id` / `api_hash` and no real-token-shaped literal in any added line; the one token literal is `internal/tgtest/tgtest.go:42`'s fake, carrying `//nolint:gosec` with a stated reason. PASS.
- AC32: `git diff --name-only c1d03fd..HEAD` contains no `*.sql` and no migration. PASS.
- D2 seam: `telego.NewBot` / `telego.With*` **call sites** exist only in `internal/tg/client.go` (`:143,:144,:145,:148,:150,:153`); every other hit is comment or failure-message text. PASS.
- Panicking-call audit over every changed non-test `.go` file → no `panic(` / `log.Fatal*` / `log.Panic*`. No `ai-docs/panic-index.md` row owed. PASS.
- Error handling / context discipline over the same set: no `_ = err`, no `context.Background()`, no `TODO`, no `<file>.md:<line>` citation in any Go file (DOC-4). Three `//nolint` directives, each naming a specific linter with a stated reason. PASS.
- Domain invariants: this diff writes no balance column and adds no posting, declares no new mechanic, adds no migration and no persisted enum, and adds no outbound send path outside the `Gate` seam AC27 covers. No telemetry or posting-signature obligation is owed. PASS.
- File sizes: largest non-test `internal/tg/limit.go` 377 / hard 1000; largest test `internal/tg/limit_test.go` 933 / hard 1500. PASS.
- Progress-file required fields (`Branch`, `base_commit`, `Last build`, `current_step`, `last_passed_gate`, `## Decisions log`) present, plus the conditional `entry_args`; `parent_skill` correctly omitted (`/task` is the parent flow). Content not reviewed, per instruction 2. PASS.
- Working tree confirmed clean (`git status --porcelain` empty) after every mutation; each mutation was applied over a `cp` backup and restored.

**Recorded, not raised** (entered in the register as `accepted@7` — `R7-2`, `R7-3`): the
Go-to-Go line citation `client.go:147-149` in `internal/tg/client_test.go:115` — DOC-4 forbids
a `.md:<line>` design citation and this is neither; and `internal/tg/retry_test.go:715,717`'s
re-resolved coverprofile coordinates, which are correct on the shipped tree but still carry no
commit pin, exactly as `R6-5`'s own cell already notes. Both are below the severity floor.
