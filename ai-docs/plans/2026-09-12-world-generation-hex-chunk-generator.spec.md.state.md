...
prior_qa:
  ...
  - round: 6
    question: |
        Саморевью дало REJECT на раунде 4 из 4. Две major — обе про слепоту одного и того же сторожа, четвёртый раунд подряд, каждый раз новая форма; живого дефекта нет. Что делаем со сторожем?
    answer: |
        Сузить заявление
  - round: 7
    question: |
        Ревью дизайна раунд 8 — ITERATE, кап снова исчерпан. Но ни одна из пяти находок не требует нового решения по дизайну — это распространение вашего же решения на четыре поверхности, исправленная посылка и три формальности. Как замыкаем гейт дизайна?
    answer: |
        Только пропагацию
```
