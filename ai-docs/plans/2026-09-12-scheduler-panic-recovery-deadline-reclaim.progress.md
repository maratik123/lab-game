...
**current_step:** Step 8 — entering Group A (subtasks 1–9)
**last_passed_gate:** check-spec-anchors.sh + check-spec-shape.sh + check-ac-shape.sh | 2026-09-12T15:41Z | 6a981f9
**entry_args:** 81
...
- [ ] 1. `internal/panicguard`: the shared recovered-panic type, its constructor from a `recover()` result, its `error` rendering, package comment, tests, leak-check `TestMain`  ← CURRENT
