...
prior_qa:
  - round: 1
    question: "Что делает игрока участником чата? Bot API не умеет перечислять участников группы, поэтому любая связь берётся только из того, что бот увидел."
    answer: "Только Start-ссылка"
  - round: 1
    question: "Если игрок состоит в нескольких чатах, из какого чата стартует рейд и как об этом спрашивают игрока?"
    answer: "По ссылке"
  - round: 1
    question: "Утверждённая спека #29 отложила представление знания чата — объединение того, что открыли его участники — до этой задачи, потому что членство, по которому идёт объединение, определяется здесь. Отдаёт ли эта задача такое представление?"
    answer: "Отдаём здесь"
```
