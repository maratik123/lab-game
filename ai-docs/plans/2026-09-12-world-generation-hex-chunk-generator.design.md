...
| 11 | **Remove the architecture gate and revert the propagation its CI job required.** This subtask landed as an *addition* before the owner's ruling arrived, so its artefacts are on the branch and the amendment makes their removal the work. **The specification is the delta, not a word list:** the gate arrived in a single commit, which is what makes a delta the whole acceptance `[measured 2d47454 · git log --oneline 4315aef..e216d5d^ -- <the paths in the Files column> → empty, and git log --oneline e216d5d..HEAD -- <same> → empty; so e216d5d is the only commit on this branch touching them]`. Restore each of those paths to its state at that commit's parent: `git diff e216d5d^..HEAD -- <path>` is the ground truth, and it catches the sites a word search cannot, which is most of them. Removing the job is the small part; the gate's commit also bumped every count and cross-reference that *named* the cluster without naming the target — the sibling-job comment in `.github/workflows/ci.yml` ("five sibling jobs", "Five jobs rather than five steps", "the other four", "none of the five"), the Race row in `ai-docs/claude-tools-hierarchy.md` ("the only one of the five"), the class-count phrasing in `.claude/skills/pr-ci-failed/SKILL.md`, and in the `Makefile` the target, its variables and their comment block. **Acceptance, run literally and per path, in whichever of two forms matches the moment it is run.** While the removal is staged and not yet committed: `git diff --cached e216d5d^ -- <path>`, which compares that commit with the **index**, so staging first is load-bearing for it. Once the removal is committed: `git diff e216d5d^..HEAD -- <path>`, which compares two **commits** and so reads neither the index nor the working tree — staging cannot change its answer, and an instruction to stage first is inert for this form. Either way the result is empty for every path except `.github/workflows/ci.yml`, where it is exactly the kept `'**/*.golden'` filter entry and its rewritten comment (§ Test Design). **A clean run over the path set is evidence about the command until a path outside the set reports non-empty**, so pair it with a control — `docs/DESIGN.md`, which subtask 12 edited, is one. *The staging concern is real and kept, attached to the checkers it actually governs:* anything walking the index or the tracked-file list cannot see a file that is not staged yet — `git diff --cached`, the comment-reference gate over the staged set, a `git ls-files` sweep — which is why the first form above names it and the second must not. `actionlint .github/workflows/ci.yml` before that file is staged, per the AXIOM; `shellcheck` is **not** engaged, the recipe body going away with the target. `ai-docs/key-decisions.md` is deliberately **not** in this subtask — KD-37 postdates the gate commit, so the delta rule would delete it; it is subtask 13's, edited in place | `Makefile`, `.github/workflows/ci.yml`, `.claude/skills/pr-ci-failed/SKILL.md`, `.claude/skills/main-ci-failed/SKILL.md`, `.claude/skills/dependabot-pr/reference.md`, `ai-docs/claude-tools-hierarchy.md`, `ai-docs/go-test-conventions.md`, `AGENTS.md` | — |
...
own non-plans paths is empty, save the exceptions the row states — which cannot be blind, because it
