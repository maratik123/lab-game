...
> **Round 2 draft.** The owner's round-1 answers settled the idempotency check's placement, the
> failure policy and the allowlist policy; those sections are now written out in full. Two items
> the answers opened are marked **PENDING Q4 / Q5**.
...
6. **Idempotency: the handler owns the basis document** (owner, round 1). The loop derives the
   ...
   `store.Post`. **`internal/store` is untouched by this task** — no post-under-an-existing-document
   API is added, and the existing phase-d refusal is the whole mechanism: on a replay `Post` returns
...
7. **Failure policy: bounded retry, then advance** (owner, round 1), mirroring the scheduler's
   ...
   - The delay between attempts grows, up to a configured cap on attempts. After the cap the loop
     **advances past the update** and records a give-up. What the give-up record consists of is
     **PENDING Q5**.
...
8. **The `ALLOWED_CHAT_IDS` gate, in the one place a handler cannot bypass.** The structural half is
   ...
   **The policy is settled** (owner, round 1): the list governs group and channel chats; **every
   private chat is allowed**, with no database read. The safety net therefore covers group traffic
   only, which is the owner's explicit trade — §1 puts every game command in DM
   [source: 118e012:docs/DESIGN.md:33-34 · `sed -n '33,34p' docs/DESIGN.md`], so a list-only gate
   would refuse the game's primary surface.

   **The mechanism by which the gate recognises a private chat is PENDING Q4** — see *Technical
   constraints* item 9 for why it is not a free choice.
...
- An enumeration API over give-up records, of the shape `scheduler.DeadTasks` has | Diagnosis
  tooling; the record itself lands here, the surface that lists it has no reader until a process
  exists to run it | yes, with the composition root
...
| Question | Decision |
|---|---|
| Where is a repeated update caught? | **At the handler's `store.Post`** (owner, round 1). `internal/store` is untouched; `ErrAlreadyPosted` is the whole mechanism; the loop classifies by sentinel and never retries a duplicate. Scope 6 states the three consequences. |
...
| What does the gate do with a chat id outside the list? | **Allow it when it is a private chat; refuse it otherwise** (owner, round 1). The list governs group and channel traffic; the net covers group traffic only, knowingly. |
...
| How does the gate recognise a private chat? | **PENDING Q4.** |
| What a give-up record consists of | **PENDING Q5.** |
...
6. **Migrations are goose files under `internal/store/migrations`, applied through `store.Migrate`,
   forward-only, with no down section.** The offset's table joins them under that discipline, and so
   does any table Q5's answer calls for.
...
8. **Propagation.** This change alters the layout of `internal/` and the standing of `cmd/bot`, and
   ...
   **If Q5's answer persists anything derived from an update**, §12.5's sanitisation obligation is a
   member of the same class: the restore script rewrites real `chat_id`/`user_id` values in one
   transaction [source: 850d97f:docs/DESIGN.md:393 · `sed -n '393p' docs/DESIGN.md`], and a new
   table holding them is a new thing for it to rewrite.

9. **The outbound seam cannot see a chat's kind — this is what makes Q4 a real question.** The gate
   receives `tg.ChatRef.Key`, the destination's raw JSON token as it appeared in the request body,
   and a `chat_id` may be a `@channelusername` string rather than a number
   ...
   Any rule that classifies an outbound token as private therefore either derives from observed
   inbound data or rests on a convention this repository cannot cite — and `AGENTS.md`
   § Dependency Versions forbids asserting external behaviour from memory. A non-numeric token is
   refused under every candidate rule: it is not in `AllowedChatIDs`, which is `[]int64`, and a
   username names a public chat, never a private one.
...
| AC21 | `internal/store`'s exported API is unchanged by this task: no new posting entry point, no new `PostingBasis` implementation, and no change to `Post`'s signature or to `ErrAlreadyPosted`'s meaning. |
...
| AC28 | The gate allows a call whose destination is a chat id in `AllowedChatIDs`; refuses a `ChatKnown` destination that is neither in the list nor classified as a private chat; and refuses a non-numeric `chat_id` token. |
...
| AC33 | *(pending Q4)* The gate classifies a private-chat destination by the rule the answer names, and the rule's basis is stated where a reader of the code meets it. |
| AC34 | *(pending Q5)* A given-up update leaves behind exactly the record the answer names, and nothing beyond it. |
...
- **Whether a give-up record ever needs to be re-injected** (a dropped update replayed by hand once
  its handler is fixed). Nothing in the MVP reads such a record; recorded so a later reader does not
