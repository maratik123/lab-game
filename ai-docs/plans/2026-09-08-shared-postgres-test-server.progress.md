...
**current_step:** Step 8 — Group A subtask 5 of 9 complete
**last_passed_gate:** `go build ./...`, `make comment-refs`; manually ran `make test` and `make test-contention CONTENTION_PARALLEL=2` end-to-end against the real podman runtime (both green; the race log fully passing, the load log's tail-end connection-refused lines are the load loop being killed after the container it depended on was torn down, not a contention finding) | pending commit
**entry_args:** 67

## Next action

**Do this immediately:** Group A, subtask 6 — `.githooks/coverage-ratchet.sh`: wrap the `go test -covermode=atomic -coverprofile ./...` measurement command with `go run ./cmd/testpg --` (D6), placed AFTER the existing raise-mode staged-check skip decision so a commit staging nothing coverage-moving still starts nothing (AC7); update the runtime advice block's "container-runtime failure" suggestion to name `make test-db-up` as the first alternative alongside the existing `export LAB_GAME_TEST_DSN=...` line. `shellcheck` and the project's help-flag-shape gate (`bash ai-docs/scripts/check-script-shape.sh`) are required on the edited script before staging.
