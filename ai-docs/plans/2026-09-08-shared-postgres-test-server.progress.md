...
**current_step:** Step 8 — Group A subtask 3 of 9 complete
**last_passed_gate:** `go build ./...`, `go vet ./...`, `golangci-lint run`, `golangci-lint fmt -d` (clean), `make comment-refs`, `go test ./...` (LAB_GAME_TEST_DSN pointed at a session-local shared container); manually smoke-tested all three decision-order paths plus `--up`/`--down` against the real podman runtime | pending commit
**entry_args:** 67

## Next action

**Do this immediately:** Group A, subtask 4 — `cmd/testpg/run_test.go`: tests over the injected `seam` (provision/probe/locate/persist/forget), exercising `run`'s decision order without a runtime — caller DSN present (seam never called), no DSN/no locator (seam's provision called, stop runs on pass/fail/interrupt), locator unreachable (ignored, provision called, message), locator reachable+undersized (provision called), locator reachable+admits (provision NOT called, no stop), caller DSN undersized (provision NOT called, shortfall reported, runs anyway), non-zero child → non-zero wrapper status, and the ceiling handed to provision matches `testdb.Ceiling(--clients, --parallel)`. No container may be started by this package's own tests (AC1).
