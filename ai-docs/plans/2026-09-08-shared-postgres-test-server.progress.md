...
**last_passed_gate:** `shellcheck -s bash .githooks/coverage-ratchet.sh`, `bash ai-docs/scripts/check-script-shape.sh`, `make comment-refs`, `.githooks/coverage-ratchet.sh --check` green against a session-local shared server | pending commit
**entry_args:** 67

## Next action

**Do this immediately:** Group A, subtask 7 — `.github/workflows/ci.yml`: add a `test-fallback` step to the Test job (D8/AC3) — bare per-package container path, so it stays exercised now that `test`/`test-race`/`cover-ratchet` all reach the shared path. Also verify (per D11/AC9) that every artefact this change added — `internal/testdb/server.go`, `internal/testdb/server_test.go`, `cmd/testpg/*.go`, the `Makefile` targets, the `.githooks/coverage-ratchet.sh` edit — is already named in the `go`/`harness` change-filter path globs; record the comparison rather than trusting the design's own claim that no filter edit is needed.
