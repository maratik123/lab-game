...
|----|--------|----------|--------|-------------------|
| R1-1 | round 1 | major | open | `grep -c chat_title internal/onboard/presence_handler_test.go` (0 = defect); mutation: blank `ChatType`/`ChatTitle`/`Date` in `presenceEventPayload`, then `go test -count=1 ./internal/onboard/` — currently exit 0, 0 FAIL |
| R1-2 | round 1 | major | open | `grep -c SELECT internal/onboard/start_handler.go` (1 = defect, 0 = fixed) |
| R1-3 | round 1 | minor | open | `grep -n 'r.Body.Read' internal/onboard/outbound_test.go` |
| R1-4 | round 1 | minor | open | `go doc ./internal/store EnsureOwner` |
