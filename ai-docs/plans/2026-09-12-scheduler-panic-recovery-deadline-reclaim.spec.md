...
3. The panic's stack is recorded on two surfaces: the task's own row carries it, and a log record carries it. [task: "its stack is recorded"] [answer 1.1: "The row carries the stack and the log carries it"]
...
| Where does a handler panic's stack land? | On both surfaces, not one: the row carries it, so it is readable without access to the logs, and a log record carries it, so it is searchable in the log stream. [answer 1.1: "The row carries the stack and the log carries it"] |
...
| AC6 | After a scheduler handler panics, the panicking call's stack is readable from the task's own row **and** from a log record the run emitted — both, so neither a reader without log access nor a reader searching the log stream is left without it. [task: "its stack is recorded"] [answer 1.1: "The row carries the stack and the log carries it"] |
...
| AC9 | After an update-ingestion handler panics, the panicking call's stack is readable from the same two surfaces AC6 names, and what ingestion reports as that update's outcome is unchanged. [answer 1.2: "Ingest's recovery records the stack the same way, in this task"] |
