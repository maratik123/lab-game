...
| Does this rework define the form a chunk's map takes in storage? | No. The owner first picked this task and then withdrew the choice as a widening of scope. Defining the stored form is not part of this rework, which keeps to the issue's own words: it exposes what persistence needs, and storing is #29's. [answer 3.2: "откатим расширение скоупа"] [task: "this issue exposes what persistence needs, storing it is #29"] |
...
| AC18 | No live document describes the world generator's chunks as the rhombic chunks this rework replaces. The class covers any such description, for example chunks sized in columns and rows or borders carrying one or two portals. The balance configuration's own comments are one such site, and the examples do not bound the class. [answer 5.3: "Amend the spec"] [task: "from rhombic chunks to hexagonal chunks"] [task: "the chunk radius key replaces the rectangular dimensions in the balance configuration and its schema"] |
