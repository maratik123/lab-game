...
| AC4 | No file in the tree, excluding the history surfaces named in § *Out of scope*, contains the literal string `subagent_type="design"` (with the closing quote, which already excludes `subagent_type="design-review"`). Every dispatch of this Subagent reads `subagent_type="design-writer"`, and the `design-review` dispatch sites are unchanged. |
