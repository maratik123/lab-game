...
maze:    Seed int64 · Algorithm (the five) · Algorithm.String() · Params
         Cell{Prefab bool; Faces; Seed uint64}
...
- **Nothing is exported for a test's benefit** — the algorithm draw and the chunk build stay
  unexported and are asserted from in-package tests. **`Algorithm.String()` is the case that looks
  like an exception and is not, so it is recorded rather than left to be discovered.** Its only
  call sites today are tests, but it satisfies `fmt.Stringer`, which makes it production behaviour
  by construction: it decides how an `Algorithm` renders in *every* formatted message, present or
  future, and no census of call sites can retract that. It also carries something the package would
  otherwise not have anywhere — the canonical spelling of the algorithm set, in the owner's own
  weight-set keys, which is the spelling the cells golden's algorithm column is written in.
  Unexporting it would break the `Stringer` contract, render the enum as an integer in any
  diagnostic, and leave that spelling defined only inside a test.
  *The boundary consequence, stated because it is a real seam rather than a clean split:*
  `internal/maze` **renders** those names and #28 **parses** them, so two tables have to agree, and
  the golden's algorithm column is what catches a drift on the rendering side.
...
| 11 | **Remove the architecture gate and revert the propagation its CI job required.** This subtask landed as an *addition* before the owner's ruling arrived, so its artefacts are on the branch and the amendment makes their removal the work. **The specification is the delta, not a word list:** the gate arrived in a single commit, which is what makes a delta the whole acceptance — pinned on ranges that do not move as the branch advances, since the obvious HEAD-relative spelling of this premise goes false the moment the removal itself lands `[measured 6a86eb9 · git log --oneline 4315aef..e216d5d^ -- <the paths in the Files column> → empty, so nothing before the gate touched them; git log --oneline e216d5d^..e216d5d -- <same> → the single line "e216d5d ci: gate world generation determinism under a second GOARCH", so the gate commit is where they all changed]`. Restore each of those paths to its state at that commit's parent: `git diff e216d5d^..HEAD -- <path>` is the ground truth, and it catches the sites a word search cannot, which is most of them. Removing the job is the small part; the gate's commit also bumped every count and cross-reference that *named* the cluster without naming the target — the sibling-job comment in `.github/workflows/ci.yml` ("five sibling jobs", "Five jobs rather than five steps", "the other four", "none of the five"), the Race row in `ai-docs/claude-tools-hierarchy.md` ("the only one of the five"), the class-count phrasing in `.claude/skills/pr-ci-failed/SKILL.md`, and in the `Makefile` the target, its variables and their comment block. **Acceptance, run literally and per path, in whichever form matches the moment it is run.** While the removal is staged and not yet committed: `git diff --cached e216d5d^ -- <path>`, which compares that commit with the **index** `[measured git 2.55.0 · git help diff → "This form is to view the changes you staged for the next commit relative to the named <commit>"]`, so staging first is load-bearing for it. Once the removal is committed: `git diff e216d5d^..HEAD -- <path>`, which compares **two commits** `[measured git 2.55.0 · git help diff → "This is to view the changes between two arbitrary <commit>." — and demonstrated at 51d274a: with an edit staged to AGENTS.md, the two-dot form over that path → 0 lines, while the --cached form over the same path → 9]` and so reads neither the index nor the working tree — staging cannot change its answer, and an instruction to stage first is inert for this form — which is why each form is stated here *with* the qualifier that belongs to it, a correct qualifier being easy to carry onto a command it does not describe. Either way the result is empty for every path except `.github/workflows/ci.yml`, where it is exactly the kept `'**/*.golden'` filter entry and its rewritten comment (§ Test Design). **A clean run over the path set is evidence about the command until a path outside the set reports non-empty, so pair it with a control** — and the control's only requirement is that it be a path already changed between `e216d5d^` and the *current* HEAD, which is what makes it discriminating; naming a file whose change this task has not reached yet would give a control that is empty for the same reason the subject is. *The staging concern is real and kept, attached to the checkers it actually governs, which are fewer than an earlier draft claimed:* `git diff --cached` and the comment-reference gate over the staged set are blind to an unstaged edit; a `git ls-files` sweep is **not** — it lists a tracked path and the grep then reads the working tree, so it sees the edit, and it is blind only to a file that is not in the index at all `[measured git 2.55.0 · in a throwaway repo, a tracked file edited but not staged → git ls-files lists it and grepping the files it names finds the unstaged text, while git diff --cached → 0 lines; a never-staged new file → git ls-files does not list it]`. `ai-docs/key-decisions.md` is deliberately **not** in this subtask — KD-37 postdates the gate commit, so the delta rule would delete it; it is subtask 13's, edited in place | `Makefile`, `.github/workflows/ci.yml`, `.claude/skills/pr-ci-failed/SKILL.md`, `.claude/skills/main-ci-failed/SKILL.md`, `.claude/skills/dependabot-pr/reference.md`, `ai-docs/claude-tools-hierarchy.md`, `ai-docs/go-test-conventions.md`, `AGENTS.md` | — |
...
- **Group C** — model `sonnet`, effort `medium` (pinned) via the `code-writer` subagent, 1M-token
  window — subtask 14 (code change-type: `*.go`). Non-terminal (1 subtask).
...
- **Group D** — model `sonnet`, effort `medium` (pinned) via the `code-writer` subagent, 1M-token
  window — subtasks 15–16 (code change-type: `*.go`). Terminal group (2 subtasks; within the
  `1..=10` range).

Four groups — **at** the default maximum, so no user gate is needed and no fifth group may be added
without one. Group D exists because the Step-11 amendment arrived after Group C had landed: its two
subtasks are code, so homogeneity bars them from the harness group, and a landed group cannot take
new work. The same constraint pair that forced Group C forces this one.
...
- **Map iteration order reaching a drawn value — and the guard against it is an approximation, which
  this row now says outright.** Any map on the generation path is a latent order dependency; the
  weight set is the obvious candidate, since a keyed mapping is its natural configuration shape.
  **First mitigation, and the one that removes the question rather than policing it:** the weight
  set is an array indexed by the enum, so that type has no iteration order and no
  unrepresentable-key validation to write.
  **Second, the structural guard — a fast pre-check on the mechanism, not a decision procedure.**
  `internal/detguard` decides map-ness by *identifier name*, gathered from a file's declarations,
  assignments, parameters, receivers, named results, struct fields and the package's named map
  types. That is a name-based approximation of a **type** question, and this design deliberately
  excludes `go/types`, so the approximation is the design, not a defect in it. Four consecutive
...
removal subtask. **The acceptance is the delta** against the tree as it stood before the gate, which
cannot be blind, because it compares against that tree rather than against a guess about how the
gate is spelled: over the gate commit's own non-plans paths the delta is empty, save the exceptions
the row states. **The command form belongs to the moment it is run and is specified in that row
only** — a commit-range form and an index form answer different questions, and this paragraph
deliberately names neither: a command spelled out on two surfaces is a command that can drift on
one of them, and the qualifier it needs travels with the form, not with the paragraph. **And the pattern keeps one job only** — as a reason, not an instrument: the class
...
interface is what keeps the permitted count at one in `internal/maze`. And no ranging over a map — **as far as a name-based predicate can see one**, which is the reach § Risks now states and whose blind shapes § Deferred carries.
