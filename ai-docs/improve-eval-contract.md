...
Three outcomes, plus one terminal state and one modifier. All of it is read from the **post-change** run alone, so it costs no extra dispatch and is available for every proposal, baseline-exempt ones included. It extends **no** blocking power: the post-change eval is already the commit gate, so every outcome other than *applied and held* already blocked before this change — the three-outcome split subdivides an **already-blocking** FAIL so Step 3 can route on it.
