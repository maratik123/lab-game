...
**Both runs are clean-context dispatches** carrying the SUBJECT block only. The forbidden degraded paths above bind the baseline dispatch exactly as they bind the verification dispatch — a same-context close-read of the pre-change state is not a baseline.
...
| **out of instrument reach** (terminal) | every attempted reproducer passed both ways, **and** a re-run of at least one of them with the append-only history excluded also passed | **Only** on explicit owner approval recorded in the PR body, with the demonstration attached | None to the RULE — the remedy is to the **instrument**. File the reach failure in `ai-docs/harness-gaps.md`, naming the class it cannot reach |
...
**The two terminal states are not synonyms, and the difference is an experiment rather than a judgement.** *No valid reproducer* says nobody built one that discriminates — it is compatible with insufficient effort, which is why the contract requires the failed attempts to be attached. *Out of instrument reach* says the class cannot be reached by clean-context dispatch **at all**, and it is earned only by running the history-excluded re-run and recording that it also passed. Without that second run the verdict is *no valid reproducer*, and the proposal does not commit. This ordering matters: the cheap verdict is the blocking one, and the unblocking verdict costs an extra dispatch plus an owner decision.
