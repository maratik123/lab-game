...
**Every scripted edit of the spec is wrapped:** `bash .claude/skills/task/scripts/doc-edit-guard.sh snapshot <spec_path>` before the edit, `… verify <spec_path>` after it, in the same command. The guard restores the file and exits 2 when a section heading, an AC row or a decision row disappeared — the shape of a heading-anchored slice that matched an in-text mention instead of the heading (it has truncated a design and a spec, both untracked at the time; `ai-docs/learnings.md` 2026-09-02 and 2026-09-08). Anchor headings on `"\n## <heading>\n"`, and rely on the guard rather than on remembering to.
...
7. Run `bash .claude/skills/ai-audit/scripts/check-spec-shape.sh <spec_path>` and `bash .claude/skills/ai-audit/scripts/check-ac-shape.sh <spec_path>`; both exit 0 or the status is not emitted. Both are CI gates, so a spec that fails them here fails the PR later.
