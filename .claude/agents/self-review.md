...
- **File size** ([`ai-docs/code-style.md` → File size](../../ai-docs/code-style.md#file-size)): any file added or grown past the **hard limit** (1000 lines excluding `_test.go` content / 1500 including) → REJECT unless exempt (generated code, one long `switch`/state machine). Crossing the **soft limit** (500 / 800) while visibly mixing responsibilities → `nit` with a split suggestion by responsibility, never by line count. Do **not** flag a cohesive medium file — one type per file is not a Go idiom.
