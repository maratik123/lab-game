...
- **Determinism asserted exactly.** A test over generation, `combat()`, or trail replay that asserts a range / "not empty" / "no error" instead of the exact seeded output → REJECT (`minor`). These are pure functions of `(seed, input)` (`~/lab-private/DESIGN.md` §2.2.2, §4) — a fuzzy assertion silently forfeits the property the design bought.
...
- **A goroutine-leak ignore entry is admitted only for a dependency's process-lifetime goroutine** (`ai-docs/go-test-conventions.md` § *Goroutine-leak detection* → Admission — the half the suite cannot judge). A `leaktest.Ignore` entry this diff adds or changes for a goroutine this module's code or a test fixture could stop — a keep-alive connection a test opened, a pool it did not close, a handler still running — → REJECT (`major`): ending that goroutine is the fix. An entry added on CI's evidence alone for a goroutine only CI's container runtime produces → REJECT (`major`), routed to the owner: that is the section's STOP, not a reviewer's waiver.
- **FSM edge coverage.** A diff touching raid-session transitions that leaves any new edge untested — **including the timer edges whose guard fails** — → REJECT (`minor`). A stale task firing late is expected traffic, not an error path (`~/lab-private/DESIGN.md` §3.5).
...
- **Goroutine ownership** ([`ai-docs/code-style.md`](../../ai-docs/code-style.md) § *Concurrency*). Every `go` statement this diff adds answers all four questions — how it stops, who waits for it, where its error goes, where its panic goes — and carries its row in the launch allow list. A goroutine started from `init()` or as a constructor's side effect; a long-lived component joined as a free goroutine rather than as a runner or a closer; a blocking operation inside a goroutine that cannot wake on cancellation; a timer or a ticker never stopped; a channel closed by other than its sender; a goroutine per inbound update with no bound; an interface-supplied handler run without a recover at that boundary → REJECT (`major`). The lint gate and the launch-allow-list guard each carry part of this class; the rest is this review's to judge.
