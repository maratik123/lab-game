...
- **The binding-constraint file for anything you specify.** Before writing "component X does Y", read the file that **CONSTRAINS** X — not a file showing X is *capable* of Y:
  ...
  - **[`ai-docs/domain-invariants.md`](../../ai-docs/domain-invariants.md)** for anything touching money, stamina, resources or items: `store.Post` owns balance mutation and the capture order, a posting group is zero-sum per kind under exactly one basis document, a new document type is a migration plus a `CHECK` edit, and a new mechanic ships its events (and posting signature) in the same PR (`docs/DESIGN.md` §13.4). A design that moves a balance without naming its basis document and its posting signature is incomplete.
...
- **A balance-moving mechanic is designed down to its postings.** Name the basis-document type, the accounts and kinds on both legs, and the posting signature the contract test will check; name the event(s) the mechanic emits (`docs/DESIGN.md` §13.4). "It debits stamina" is not a design.
- **Balance numbers are configuration, and the design says so.** When a mechanic needs a tuning value (a cost, a timer, a rate, a curve), the design specifies the **config key and its shape**, never a literal to compile in (`docs/DESIGN.md` §16.5). If the design document leaves the number open (§16.5 leaves most of them open), that is not a blocker — spec the key, pick a placeholder, and say it is a placeholder.
