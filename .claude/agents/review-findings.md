...
- **File size** ([`ai-docs/code-style.md` → File size](../../ai-docs/code-style.md#file-size)): any non-exempt `.go` file over the **hard limit** (1000 lines excluding `_test.go` content / 1500 including) → `major`, refactor required. Over the **soft limit** (500 / 800) while visibly mixing responsibilities → `minor` with a split-by-responsibility suggestion. Exemptions: generated code, one long `switch`/state machine. Do **not** flag a cohesive medium file — one type per file is not a Go idiom.
