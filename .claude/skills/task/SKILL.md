...
Run the full verify list in `reference.md` § Step 9 — verify list (full): `go build ./...`, `go test ./...`, `go test -race ./...` (when the change touches goroutines / the scheduler / shared state), `golangci-lint fmt -d`, `golangci-lint run`, `go vet ./...`, `go mod tidy` + `git diff --exit-code go.mod go.sum` (only if dependencies moved), `make comment-refs`, `make import-guard`, `actionlint` (only if workflows changed), `shellcheck` (only if a shell script changed), panic-index sync (see `reference.md` § Step 9 — panic-index sync (detail)), domain-invariant sweep (see `reference.md` § Step 9 — domain-invariant sweep), then per-AC coverage check and a `| # | Criterion | Test / Verification | Status |` summary table. On ALL PASS → Step 9.5.
...
**Who authors a fix (binding).** A fix whose diff is predominantly `.go` — with its `_test.go`, `go.mod` and migrations — is authored by `code-writer` in Mode B: the orchestrator plans it, the delegate authors it, runs its gates and returns without committing, and the orchestrator reads the diff and commits. A predominantly-prose fix is authored in-thread. Step 8's rule that the orchestrator never executes subtask code in its own context holds here too: `ai-docs/delegation-rules.md` § Phase 1 — Fit is the rule, and `ai-docs/learnings.md` 2026-09-13 records the `.go` batch that was authored in-thread without it.
...
After all findings are resolved, run gates (`go build ./...`, `go test ./...`, `golangci-lint run`) **and the fix-round measurement pass (binding):** execute the `verifying command` of every register row this round touched, plus every AC whose measurement the touched files could move (at minimum: every AC the design lists a verification command for over a file in this round's diff), quoting outputs into the register. Then:
