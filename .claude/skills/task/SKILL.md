...
> The `/task` orchestrator NEVER writes to `ai-docs/plans/*.spec.md` or `ai-docs/plans/*.design.md` (including `done/` siblings). All such writes go through the responsible Subagent: `spec-writer` for `*.spec.md`, `design` for `*.design.md`. Orchestrator-side direct edits with `Edit` / `Write` are FORBIDDEN.
...
**Every number and every `file:line` you write here is a measurement, not a recollection** — re-derive each one in this turn, after the last edit, with the command that produces it; **NEVER** transcribe a figure from a subagent's return summary (a claim, AGENTS.md § *Workflow*); a new figure that contradicts an existing durable baseline is resolved by a fresh measurement, never by picking one of the two.
