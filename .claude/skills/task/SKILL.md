...
Run the full verify list in `reference.md` § Step 9 — verify list (full): `go build ./...`, `go test ./...`, `go test -race ./...` (when the change touches goroutines / the scheduler / shared state), `golangci-lint fmt -d`, `golangci-lint run`, `go vet ./...`, `go mod tidy` + `git diff --exit-code go.mod go.sum` (only if dependencies moved), `make comment-refs`, `actionlint` (only if workflows changed), `shellcheck` (only if a shell script changed), panic-index sync (see `reference.md` § Step 9 — panic-index sync (detail)), domain-invariant sweep (see `reference.md` § Step 9 — domain-invariant sweep), then per-AC coverage check and a `| # | Criterion | Test / Verification | Status |` summary table. On ALL PASS → Step 9.5.
