...
1. **Classify each note** at Step 7 close: **design-internal** (fold into the design doc in-place; no loop — current behaviour) vs **spec-amending** — and a note is spec-amending **only** when it is one of the four triggers of `SKILL.md`'s AXIOM *the orchestrator originates no spec row* (a missing row the task's words require; an unsatisfiable or contradictory row; a row outside the spec's zone; the owner's own words). A note about code the task merely touches is design-internal or a follow-up, whatever its severity. "The spec does not allow the design's fix" is trigger (c) against the blocking row — restate it as the outcome it protects, or strike it — never a widening that writes the fix into the spec. Mixed batches are allowed; spec-amending notes trigger this recipe, design-internal notes proceed normally.
...
3. **Surface to user via `AskUserQuestion`** — name the trigger, quote the row or rows and the reason, and propose the new wording as an outcome; wait for explicit approval. **The spec is never amended to match the shape the design discovered:** a mechanism the design chose stays in the design, where the design's own rounds judge it. The question carries its route (`SKILL.md` AXIOM *a scope question carries its route*) and never offers "do the work now" and "amend the spec" as one option.
4. **On user approval — amend the spec via the `spec-writer` Subagent.** The orchestrator MUST NOT edit `*.spec.md` directly (per the AXIOM in `SKILL.md` above the Design Amendment header). **First append the question and the owner's verbatim answer to `prior_qa` in the state file under the next round number, and advance `round`** — the amended row anchors to that entry (`spec-writer.md` Rule 11), and without it `check-spec-anchors.sh` refuses the row. Then spawn `spec-writer` for that synthetic round (`extra_context` carries the amendment description; the anchor is the `prior_qa` entry, not this field); the Subagent re-writes the spec on disk. Orchestrator-side direct `*.spec.md` edits with `Edit` / `Write` are FORBIDDEN — mirrors `.claude/skills/interview/SKILL.md` § Anti-patterns ("Mutating the spec yourself").
...
**A `file:line` you must write anyway** — a panic-index row locator is the live case — is re-derived **in this turn, after the last edit**, with the command that produces it (the locator `rg` re-run after the final `golangci-lint fmt`). **NEVER** transcribe it from a subagent's return summary: it is a claim (AGENTS.md § *Workflow*), and a `code-writer` has already reported one package's figure for another's. If it contradicts an existing durable baseline, the resolution is a **fresh measurement**, never picking one of the two.
...
9. **`make file-limits`** — clean. CI's Lint job runs it, and no other gate on this list covers it: `golangci-lint run` stays green on a file that breaks the 1000 / 1500-line limit, so skipping this one records `ALL PASS` on a tree CI will reject.
9a. **`make comment-refs`** — clean. No comment in a gated file (`*.go`, `*.sh`, `*.sql`, `*.yml`, `*.yaml`, `.gitignore`, `.env.example`, `Makefile`, `.githooks/**`) carries an outward reference. Exit 2 is an instrument failure, not a pass. Running `make verify` discharges items 1–8, 9, this one and 9b together.
9b. **`make import-guard`** — clean. No package the rule table names reaches a forbidden module through its **non-test** dependency graph. A `_test.go` importer is legal by construction, so a finding is a real production import, and the fix is the import — never the rule table.
...
| Step 9 | `go build ./...` ✅? `go test ./...` green? `go test -race ./...` green when the change touches goroutines / the scheduler / shared state? `golangci-lint fmt -d` clean? `golangci-lint run` clean? `go vet ./...` clean? `go mod tidy` leaves `go.mod`/`go.sum` unchanged (only if deps moved)? `make file-limits` clean (no other gate here covers it — `golangci-lint run` stays green on an over-limit file)? `make comment-refs` clean? `make import-guard` clean? `actionlint` clean on every changed workflow and `shellcheck` clean on every changed script (skip if none)? Any new `panic(` / `log.Fatal*` / `Must…` outside `_test.go` → `ai-docs/panic-index.md` updated and staged? Domain-invariant sweep run, every hit resolved or justified in the decisions log? All ACs covered? |
