...
9a. **`make comment-refs`** — clean. No comment in a gated file (`*.go`, `*.sh`, `*.sql`, `*.yml`, `*.yaml`, `.gitignore`, `.env.example`, `Makefile`, `.githooks/**`) carries an outward reference. Exit 2 is an instrument failure, not a pass. Running `make verify` discharges items 1–8, 9 and this one together.
...
| Step 9 | `go build ./...` ✅? `go test ./...` green? `go test -race ./...` green when the change touches goroutines / the scheduler / shared state? `golangci-lint fmt -d` clean? `golangci-lint run` clean? `go vet ./...` clean? `go mod tidy` leaves `go.mod`/`go.sum` unchanged (only if deps moved)? `make file-limits` clean (no other gate here covers it — `golangci-lint run` stays green on an over-limit file)? `make comment-refs` clean? `actionlint` clean on every changed workflow and `shellcheck` clean on every changed script (skip if none)? Any new `panic(` / `log.Fatal*` / `Must…` outside `_test.go` → `ai-docs/panic-index.md` updated and staged? Domain-invariant sweep run, every hit resolved or justified in the decisions log? All ACs covered? |
