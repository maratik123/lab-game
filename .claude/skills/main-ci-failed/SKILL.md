...
> **CI exists** (`.github/workflows/ci.yml`): Format · Build · Test · Race · Coverage ratchet · Test fallback · Lint · Harness guards · Actionlint, each `paths-filter`-gated. This skill acts on a red run on `main`. Note that `origin` requires a check only if a ruleset says so (`AGENTS.md` § Permissions); without one a red main is caught by watching, not by a blocked merge.
