...
Fires BEFORE Step 5 when the round's diff touches `ai-docs/plans/*.spec.md` (or `done/*.spec.md`). Mirrors the `/task` Step 7 *Spec Amendment recipe* rule — same root cause, same remedy in every downstream "fix" skill.
...
3. Only on a GO verdict from design-review: resume `/pr-commented` Step 5 (self-review). `self-review` operates on a code-vs-spec diff; it cannot validate that the spec → design → implementation chain still holds after a spec amendment — that's what the design-review re-entry does. The GO's notes route by `/task` Step 7's table: design-internal ones are folded in without another design-review; a spec-amending one goes to the owner's three options.
