...
1. The diff touches files beyond `go.sum` despite CI being green. This is unusual for Dependabot gomod PRs — Dependabot may have updated `go.mod` in addition to `go.sum`, or a maintainer may have pushed an extra commit to the branch via `--allow-edits-from-maintainers`.
...
**Class:** <fmt|build|tidy|test|race|lint|harness|actionlint>
...
1. **Pin** the module at the pre-bump version in `go.mod`; close PR
   #<N>.
...
files beyond `go.sum`. Expected diff shape for a Dependabot gomod
PR: `go.sum` only. Observed:
...
- Dependabot updated `go.mod` in addition to `go.sum` (common
...
Not posted on the PR — `/dependabot-pr` v1 bails at **preconditions** for non-gomod Dependabot branches, before the PR is touched. The bail message goes to the user only:

```
PR #<N> is a Dependabot `<ecosystem>` PR (branch: `<headRefName>`).
/dependabot-pr v1 supports gomod only. Handle manually or wait
...
```
PR #<N> Dependabot gomod bump — READY TO MERGE.

Diff scope: lockfile-only
CI state: all-green (<n> required checks PASS)
Local validation: go build ./... clean (lockfile resolves)
