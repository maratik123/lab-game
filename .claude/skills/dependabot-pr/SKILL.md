...
Workflow for **one round** of Dependabot-PR triage on the current branch's (or explicitly-referenced) open Dependabot gomod PR. Steps execute strictly in sequence. Re-invocable: call again after Dependabot rebases, after CI re-runs, or after the user merges and moves to the next bot PR.
...
- Snapshotting the open Dependabot gomod PR (title, body, files-changed, labels, check-runs, commits).
...
- Handling non-Dependabot bot PRs (`renovate[bot]`, `pre-commit-ci[bot]`, …) or Dependabot **github-actions** PRs (v1 is gomod-only). Preconditions bail on these.
...
| Current branch matches `dependabot/go_modules/...` OR caller passed an explicit PR number that resolves to a Dependabot gomod PR | branch does NOT start with `dependabot/go_modules/` AND no PR-number argument provided. |
| Ecosystem is gomod | branch starts with `dependabot/github_actions/` (or any other non-gomod Dependabot ecosystem). v1 bails — out of scope. |
...
If the branch is `dependabot/github_actions/*` (or another non-gomod ecosystem), bail with the message: *"PR #<N> is a Dependabot `<ecosystem>` PR. `/dependabot-pr` v1 supports gomod only. Handle manually or wait for v2 ecosystem support."*
...
- **`lockfile-only`** — every file in `gh pr view --json files` matches `go.sum` (no other paths touched).
- **`scope-drift`** — any file other than `go.sum` appears in the diff (including `go.mod`, source files, READMEs, workflow YAML, …).
...
- **print-merge-command-and-pause.** Local `go build ./...` validates the lockfile. On success, print the `gh pr merge --merge <N>` command (NEVER `--squash` / `--rebase`) and pause. The user runs the merge.
...
| Step | Gate |
|---|---|
| Preconditions | Branch ≠ main; tree clean; PR open, authored by `dependabot[bot]`, targets `main`; ecosystem = gomod; branch matches or explicit PR number provided |
...
| Step 4 | Terminal action executed: `go build ./...` clean (for confirm-merge) OR child `/pr-ci-failed` returned and verdict translated OR bail-comment / issue posted OR pause-for-user message printed |
