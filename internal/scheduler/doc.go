...
// transaction (docs/DESIGN.md §11). A task type is a Go registry entry,
...
// a persisted instant.
//
// This package never imports internal/store: it moves no balance of its
// own, and the code that posts a mechanic's effects is that mechanic's
// own handler, given the worker's own *pgx.Tx.
//
// A scheduled task's payload is a data contract (AGENTS.md § API
// Stability carve-out): a payload key, once shipped, is added to or left
